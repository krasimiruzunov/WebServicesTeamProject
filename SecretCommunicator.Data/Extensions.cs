...
namespace SecretCommunicator.Data
{
    public static class Extensions
    {
        ...
        {
            if (
                AppCache.MessageList.Where(m => m.ChannelId == chan.Id)
                    .OrderBy(m => m.CreatedDateTime)
                    .Take(numberOfMessages)
                    .ToList()
                    .Count == 0)
            {
                _sessionState.GetMessages()
                    .Where(m => m.ChannelId == chan.Id)
                    .OrderBy(m => m.CreatedDateTime)
                    .Take(numberOfMessages)
                    .ToList()
                    .ForEach(m =>
            ...
            chan.Messages = AppCache.MessageList.Where(m => m.ChannelId == chan.Id)
                .OrderBy(m => m.CreatedDateTime)
                .Take(numberOfMessages)
                .ToList();
        }
        ...
        public static Channel GetChannel(this SessionState _sessionState, string name, string password, PubnubAPI pubnub)
        {
            ...
            if (result == null)
            {
                ...
                if (result == null)
                {
                    ...
                    if (pubnub != null)
                    {
                        List<object> publishAllChannel = pubnub.Publish("SCAllChannel", result);
                    }
                }
