    ...
    public class ChannelController : BaseApiController<Channel>, IRequiresSessionState
    {
        ...
        public override HttpResponseMessage Post(Channel value)
        {
            ...
            try
            {
                ...
                {
                    channel = _sessionState.GetChannel(name);
                    if (channel != null)
                    {
                ...
                {
                    channel = _sessionState.GetChannel(name, password, _sessionState.Pubnub);
                    if (channel != null)
                    {
                ...
                response.Headers.Location = new Uri(Url.Link("DefaultApi", new {id = channel.Id}));
                return response;
            }
            catch (Exception ex)
            {
